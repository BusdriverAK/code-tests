...
function sumArray(sumArr) { //eslint-disable-line
    function sumNum(sumArr){
        ...
        for (var i = 0; i < sumArr.length; i++){
            sumVal = sum(sumVal, sumArr[i])[0];
        }
        return sumVal;
    }
    return [sumNum(sumArr), sumArr + ' was passed in as an array of numbers, and ' + sumNum(sumArr) + ' is their sum.'];
}
...
function multiplyArray(multArr) { //eslint-disable-line
    function multArrNum(multArr) {
        var ProductNum = 1;
        for(var i = 0; i < multArr.length; i++) {
            ProductNum = multiply(ProductNum, multArr[i])[0];
        }
        return ProductNum;
      }
      return [multArrNum(multArr), 'The numbers ' + multArr + ' have a product of ' + multArrNum(multArr) + '.'];
}
